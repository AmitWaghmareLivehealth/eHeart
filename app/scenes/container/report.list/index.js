...
class ReportList extends Component {
        ...
        if (result.success) {
          ...
          setTimeout(() => {
            this.setNewDataSource(
              result.response.allReportSortedCatMap,
              result.response.activeUserSection,
            );
          }, 50);
        }
      })
  ...
  setNewDataSource(allReportSortedCatMap, activeUserSection) {
    ...
    allKeys.forEach(key => {
      ...
      if (
        neededKey.length > 1 &&
        Object.keys(allReportSortedCatMap[key]).length > 0
      ) {
  ...
  async mergeReport() {
    ...
    let selectedToRoute = this.state.selectedToRoute.key
      ? this.state.selectedToRoute.key
      : this.state.activeUserSection;
    let selectedReportSection =
      Object.keys(this.state.dataSourceArray[selectedToRoute])[0] || '';
    var selectedSectionArrayPrimary = selectedReportSection.split(
      Global.splitter,
    );
    var primaryUserDetailsId =
      selectedSectionArrayPrimary[selectedSectionArrayPrimary.length - 1];

    var selectedSectionArraySec = this.state.selectedSectionId.split(
      Global.splitter,
    );
    var secondaryUserDetailsId =
      selectedSectionArraySec[selectedSectionArraySec.length - 1];

    if (
      !['', 0, '0'].includes(primaryUserDetailsId) &&
      !['', 0, '0'].includes(secondaryUserDetailsId)
    ) {
      var _this = this;
      ReportManager.mergeReport(
        primaryUserDetailsId,
        secondaryUserDetailsId,
        _this,
      )
        .then(result => {
          if (result.success) {
            ...
            AlertManager.AlertGeneric(
              'Success',
              'The reports has been merged successfully.',
            );
          }
        })
        .catch(error =>
  ...
  async detachReport() {
    this.setState({isLoading: true});
    let sectionArray = this.state.dataSourceArray[this.state.activeUserSection][
        ...
        ReportManager.detachReport(selectedItem.billId.id, _this)
          .then(result => {
            if (result.success) {
              ...
              var newSectionId = this.state.selectedSectionId.replace(
                selectedSectionArraySec[selectedSectionArraySec.length - 1],
                newUserDetailsId + '',
              );
              var newSectionArray = [];
              ...
              delete newDataSourceArray[this.state.activeUserSection][
                this.state.selectedSectionId
              ];
              if (
                !newDataSourceArray[
                  selectedItem.fullName + Global.splitter + newUserDetailsId
                ]
              ) {
    ...
    return (
      <Ripple
        ...
        <View style={[styles.containerRowMain]}>
          <Text style={[CommonStyles.textDescription2, styles.textReportName]}>
            {item.reportName || item.title || ''}
          </Text>
          {((item.uploadDate || '').length > 3 ||
            (parseInt(item.isFileInputReport, 10) || 0) === 1) && (
            <MaterialIcons
              name={'attachment'}
              size={16}
  ...
  goToReportView(navigateToReportUpload = false, isComingFromFab) {
    ...
    setTimeout(
      (() => {
        ...
        if (
          (selectedItem.uploadDate || '').length > 0 ||
          navigateToReportUpload
        ) {
          ...
          let sectionArray = this.state.dataSourceArray[
            this.state.activeUserSection
          ][this.state.selectedSectionId];
          if (
            (selectedItem === {} || isComingFromFab) &&
  ...
  _renderSections(sectionData, category) {
    var regDate =
      moment(
        ((sectionData[0].userDetailsId || {}).user || {}).date_joined || '',
        Global.LTHDateFormatMoment + 'Z',
      ).format(Global.dateFormatDisplay) || '';
    var years =
      moment().diff(
        moment(
          (sectionData[0].userDetailsId || {}).dateOfBirth || '',
          'YYYY-MM-DD',
        ),
        'years',
        false,
    ...
    var isSelfOwnedReports =
      reportOwner !== this.state.activeUserSection.split(Global.splitter)[0];
      ...
      if (isSelfOwnedReports || this.state.isFirstAndOnlyUserIndirect) {
        return (
          <View
            ...
            (!this.state.isFirstAndOnlyUserIndirect &&
              !viewAllReportVisible) ? (
    ...
    return (
      <View style={styles.containerDateAndLabNameHeader}>
        <Text
          style={[CommonStyles.textHeader3, styles.textDateAndLabNameHeader]}>
          {labName}
        </Text>
        <Text
          style={[
            CommonStyles.textDescription3,
            styles.textDateAndLabNameHeader,
          ]}>
          {date}
        </Text>
        {selfOwnedReports()}
    ...
    return (
      <ListView
        ...
        style={{
          ...
          height: this.state.scrollContentHeight2,
  ...
  _renderModal() {
    ...
    var validDate =
      (mergeInfo.regDate || '').length > 0 &&
      !(mergeInfo.regDate || '').toUpperCase().includes('INVALID');
        ...
        <View
          ...
          <View
            ...
            <View
              ...
              <Text
                style={[
                  CommonStyles.textHeader4,
                  styles.title_style,
                  {marginRight: 12},
                ]}>
                {mergeInfo.reportOwner}
              </Text>

              {genderAndAgeString().length > 0 && (
                <Text
                  style={[
                    CommonStyles.textDescription1,
                    {marginBottom: 7, marginRight: 12},
                  ]}>
                  {genderAndAgeString()}
                </Text>
              )}
            ...
            {(mergeInfo.labName || '').length > 0 && (
              <Text
                ...
                numberOfLines={2}
                ellipsizeMode={'tail'}>
                Registered at {mergeInfo.labName}
              </Text>
            )}
            {validDate && (
              <Text
          ...
          <View
            style={{backgroundColor: 'white', marginLeft: 24, marginRight: 24}}>
            {/*<Separator />*/}
            ...
            {mergeInfo.mergeReportVisible && (
              <View>
                <ButtonTextOnly
                  ...
                  titleStyle={[
                    CommonStyles.textHeader3,
                    {fontSize: 18, fontWeight: '400', color: Color.theme_blue},
                  ]}
                ...
                <Text
                  ...
                  Incase of duplicate users, use this option to merge reports
                  from one user to another.
                </Text>
                <Separator style={{marginLeft: 24, marginRight: 24}} />
              </View>
            )}
            {mergeInfo.separateReportVisible &&
              !this.state.isFirstAndOnlyUserIndirect && (
                <View>
                  <ButtonTextOnly
                  ...
                  <Text
                    ...
                    Incase of report of family members, use this option to
                    separate reports.
                  </Text>
                  {/*<Separator />*/}
                </View>
    ...
    return (
      <ModalNormal
        ...
        actionList={[
          ...
          {title: 'Cancel', onPress: this.onMergeCancel, enable: true},
        ]}>
        <View>
          <View style={{margin: 24, marginBottom: 14}}>
            <Text style={[CommonStyles.textHeader4, styles.title_style]}>
              Merge Reports
            </Text>
            <Text
              style={[
              ...
              Incase of duplicate users, use this option to merge reports from
              one user to another.
            </Text>
          </View>
          <Separator />
          <View
            ...
            <Text style={{fontFamily: 'Arial', padding: 5, fontWeight: '700'}}>
              {this.state.mergeInfo.reportOwner}
            </Text>
            {/*{this.state.tabState.routes && this._renderSelect('from', selectedToRoute)}*/}
            <View style={{height: 17}} />
            {this.state.tabState.routes &&
              this._renderSelect('to', selectedFromRoute)}
          </View>
          <Separator />
        </View>
      </ModalNormal>
    ...
    return (
      <View>
        ...
        <Select
          ...
          style={{
            borderWidth: 1.0,
            borderColor: Color._EEGrayTableHeader,
            backgroundColor: 'white',
          }}
          textStyle={{padding: 3}}
          transparent
  ...
  onSelect(option, element) {
    console.log(
      'From -> ',
      this.state.selectedFromRoute,
      this.state.selectedToRoute,
    );
    switch (option) {
      case 'from':
        this.setState({selectedFromRoute: element});
        break;
      case 'to':
        this.setState({selectedToRoute: element});
        break;
      default:
        break;
    }
  ...
  onMergeCancel() {
    this.setState({
      isMergeReportDialogVisible: false,
      selectedFromRoute: undefined,
      selectedToRoute: undefined,
    });
  }
          ...
          !this.state.isPreLoading ? (
            reportPresent ? (
              <View
                ...
                {!this.state.isLoading && this.state.isTabNeeded ? (
                    ...
                    <ScrollableTabs
                      ...
                      onChangeTab={(page => {
                        this.setState({
                          ...
                          activeUserSection: this.state.tabState.routes[page.i]
                            .key,
                        });
                      }).bind(this)}
                      style={{backgroundColor: 'white'}}>
                      {allTabsArray.length > 0 &&
                        allTabsArray.map(element => {
                  ...
                  <Animated.View style={{flex: 1, bottom: anim}}>
                    {this._renderTabScene({
                      route: {key: this.state.activeUserSection},
                    })}
                  </Animated.View>
                )}
                {this.state.isReportSecDetailsDialogVisible &&
                  this._renderModal()}
                {this.state.isMergeReportDialogVisible &&
                  this._renderModalMerge()}
          ...
          <Modal
            ...
            {this.state.upload_newreportModal ? (
              ...
              <ReportView
                ...
                userName={
                  this.state.activeUserSection.split(Global.splitter)[0]
                }
                setReportReadCount={this.setReportReadCount}
                closeModal={this.closeModal}
